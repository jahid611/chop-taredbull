...
import { User, LogOut, ShoppingBag, Settings, Bell, Shield, CreditCard, MapPin, Package } from 'lucide-react';
...
const Profile = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Livré': return 'text-green-500';
      case 'En cours': return 'text-blue-500';
      case 'En préparation': return 'text-yellow-500';
          ...
          <Card className="lg:col-span-1 bg-gray-800/50 border-gray-700">
            <CardContent className="p-6">
              <div className="flex flex-col items-center text-center">
                <div className="w-24 h-24 rounded-full bg-gradient-to-r from-emerald-400 via-purple-500 to-pink-500 p-1">
                  <div className="w-full h-full rounded-full bg-gray-800 flex items-center justify-center">
                    <User className="w-12 h-12 text-white" />
                  </div>
                </div>
                <h2 className="mt-4 text-xl font-bold text-white">{user.prenom} {user.nom}</h2>
                <p className="text-gray-400">{user.email}</p>
                <p className="mt-2 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gradient-to-r from-emerald-400/10 to-purple-500/10 text-emerald-400">
                  {user.role}
                </p>
              </div>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <h3 className="text-2xl font-bold text-white">649.97 €</h3>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        {orders.map(order => (
                          <div key={order.id} className="flex items-center justify-between p-4 rounded-lg bg-gray-700/50">
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <p className="text-sm font-medium text-white">Commande #{order.id}</p>
                ...
                <Card className="bg-gray-800/50 border-gray-700">
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      {orders.map(order => (
                        <div key={order.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-lg bg-gray-700/50">
                          <div className="flex items-center space-x-4">
                            <div className="bg-gray-600 p-3 rounded-lg">
                              <Package className="w-6 h-6 text-white" />
                            </div>
                            <div>
                              <p className="text-lg font-medium text-white">Commande #{order.id}</p>
                              <p className="text-sm text-gray-400">{order.date}</p>
                            </div>
                          </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <p className="text-sm font-medium text-white">Notifications par email</p>
                            <p className="text-sm text-gray-400">Recevez des mises à jour sur vos commandes</p>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-400">Gérez vos paramètres de sécurité</p>
                          </div>
                        </div>
                        <Button variant="outline" size="sm">Configurer</Button>
                      </div>
